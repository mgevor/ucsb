 ...
 */
class speedb_transaction_t : public ucsb::transaction_t {
  public:
    inline speedb_transaction_t(std::unique_ptr<rocksdb::Transaction> transaction,
...
speedb_transaction_t::~speedb_transaction_t() {
...
operation_result_t speedb_transaction_t::upsert(key_t key, value_spanc_t value) {
...
operation_result_t speedb_transaction_t::update(key_t key, value_spanc_t value) {
...
operation_result_t speedb_transaction_t::remove(key_t key) {
    auto key_slice = to_slice(key);
...
operation_result_t speedb_transaction_t::read(key_t key, value_span_t value) const {
    rocksdb::PinnableSlice data;
    rocksdb::Status status = transaction_->Get(read_options_, to_slice(key), &data);
    if (status.IsNotFound())
        return {0, operation_status_t::not_found_k};
...
operation_result_t speedb_transaction_t::batch_upsert(keys_spanc_t keys,
...
operation_result_t speedb_transaction_t::batch_read(keys_spanc_t keys, values_span_t values) const {
...
operation_result_t speedb_transaction_t::bulk_load(keys_spanc_t keys,
                                                   values_spanc_t values,
                                                   value_lengths_spanc_t sizes) {
    return batch_upsert(keys, values, sizes);
}

operation_result_t speedb_transaction_t::range_select(key_t key, size_t length, values_span_t values) const {
...
operation_result_t speedb_transaction_t::scan(key_t key, size_t length, value_span_t single_value) const {

    size_t i = 0;
    rocksdb::ReadOptions scan_options = read_options_;
    // It's recommended to disable caching on long scans.
    // https://github.com/speedb/rocksdb/blob/49a10feb21dc5c766bb272406136667e1d8a969e/include/rocksdb/options.h#L1462
