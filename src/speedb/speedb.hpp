...
#include "speedb_transaction.hpp"

namespace ucsb::speedb {
...
/**
 * @brief speedb wrapper for the UCSB benchmark.
 * https://github.com/speedb/speedb
 *
 * Warning: Use custom keys comparator or swap byte from
 * little-endian to big-endian via `__builtin_bswap64`.
 */
class speedb_t : public ucsb::db_t {
  public:
    inline speedb_t(db_mode_t mode = db_mode_t::regular_k)
...
void speedb_t::set_config(fs::path const& config_path,
                          fs::path const& main_dir_path,
                          std::vector<fs::path> const& storage_dir_paths,
...
bool speedb_t::open(std::string& error) {
    if (db_)
        return true;
...
operation_result_t speedb_t::upsert(key_t key, value_spanc_t value) {
...
operation_result_t speedb_t::update(key_t key, value_spanc_t value) {
...
operation_result_t speedb_t::remove(key_t key) {
    rocksdb::Status status = db_->Delete(write_options_, to_slice(key));
    return {size_t(status.ok()), status.ok() ? operation_status_t::ok_k : operation_status_t::error_k};
}

operation_result_t speedb_t::read(key_t key, value_span_t value) const {
    rocksdb::PinnableSlice data;
...
operation_result_t speedb_t::batch_upsert(keys_spanc_t keys, values_spanc_t values, value_lengths_spanc_t sizes) {
...
operation_result_t speedb_t::batch_read(keys_spanc_t keys, values_span_t values) const {
...
operation_result_t speedb_t::bulk_load(keys_spanc_t keys, values_spanc_t values, value_lengths_spanc_t sizes) {
    size_t idx = 0;
...
operation_result_t speedb_t::range_select(key_t key, size_t length, values_span_t values) const {
...
operation_result_t speedb_t::scan(key_t key, size_t length, value_span_t single_value) const {

    size_t i = 0;
    rocksdb::ReadOptions scan_options = read_options_;
    // It's recommended to disable caching on long scans.
    // https://github.com/speedb/speedb/blob/49a10feb21dc5c766bb272406136667e1d8a969e/include/speedb/options.h#L1462
...
std::string speedb_t::info() {
    return fmt::format("v{}.{}", rocksdb::kMajorVersion, rocksdb::kMinorVersion);
}
...
size_t speedb_t::size_on_disk() const {
...
std::unique_ptr<transaction_t> speedb_t::create_transaction() {
    ...
    return std::make_unique<speedb_transaction_t>(std::move(raw), cf_handles_);
}

bool speedb_t::load_additional_options() {
